...
import ms from "./multiselect.module.css";
...
export class Multiselect extends React.Component {
    ...
    renderNormalOption() {
        const { isObject = false, displayValue, showCheckbox, style, singleSelect, level } = this.props;
        const { highlightOption } = this.state;
        return this.state.options.map((option, i) => (
            <li
                key={`option${i}`}
                style={level && option[level] ? {paddingLeft: option[level]*30+"px"}: {}}
                className={`
    ...
    renderMultiselectContainer() {
        ...
        return (
            <div className={`multiselect-container ${ms.multiSelectContainer} ${disable ? `${ms.disable_ms} disable_ms` : ''}`} id={id || 'multiselectContainerReact'} style={style['multiselectContainer']}>
                ...
                <div
                    className={`optionListContainer ${ms.optionListContainer} ${toggleOptionsList ? ms.displayBlock : ms.displayNone
                        }`}
                >
